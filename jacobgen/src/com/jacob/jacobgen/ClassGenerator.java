...
/**
 * The standard class generator for DLL entities of type TKIND_COCLASS and
 * TKIND_DISPATCH
 * 
 * @version $Id$
 * 
...
class ClassGenerator extends AbstractGenerator {
	...
	public static String[] javaKeywords = { "goto", "default", "volatile",
			"import", "new", "abstract" };

	protected boolean containsDate = false;

	protected ClassGenerator(String filename, String typelibName,
	...
	protected void writeFields() throws IOException {
		...
		if (guid == null || "<unknown>".equals(guid)) {
			w.write("\tpublic static final String componentName = \""
					+ typelibName + "." + className + "\";\n\n");
		} else {
			w.write("\tpublic static final String componentName = \"clsid:"
					+ guid + "\";\n\n");
		}
	}
	...
	protected void writeConstructor2() throws IOException {
		w.write("\t/**\n");
		w
				.write("\t* This constructor is used instead of a case operation to\n");
		w
				.write("\t* turn a Dispatch object into a wider object - it must exist\n");
		w
				.write("\t* in every wrapper class whose instances may be returned from\n");
	...
	protected void writeMethods() throws IOException {
		Enumeration<MethodItem> e = classMethods.elements();
		while (e.hasMoreElements()) {
			MethodItem mi = e.nextElement();
	...
	/**
	 * This Method writes a short javadoc-comment in front of the method. Maybe
	 * in a future version the helpstring could be extracted an used instead the
	 * current weak description.
	 * 
	 * @param mi
	 *            the MethodItem which should be documented
	 * @param paramNum
	 *            the count of parameters
	 * @param baseTypes
	 *            is this a comment for method with only base types or is this a
	 *            comment for method with output-parameters
	 * @throws IOException
	 *             the used Writer could throw an IOException
	 */
	protected void writeMethodComment(MethodItem mi, int paramNum,
		...
		if (baseTypes) {
			w
					.write("\t * Wrapper for calling the ActiveX-Method with input-parameter(s).\n");
		} else {
			w
					.write("\t * Wrapper for calling the ActiveX-Method and receiving the output-parameter(s).\n");
		}
		...
		for (int i = 0; i < paramNum; i++) {
			...
			if (baseTypes || p.getDirection() == ParameterItem.DIRECTION_IN
					|| p.getDirection() == ParameterItem.DIRECTION_UNKNOWN) {
				w.write("\t * @param " + p.getJavaName()
						+ " an input-parameter of type " + p.getType() + "\n");
			} else {
				// this is only necessary if we want to comment non-basetypes
				// and if it is an output-parameter
				...
				w
						.write("\t * @param "
								+ p.getJavaName()
								+ " is an one-element array which sends the input-parameter\n"
								+ "\t *        "
								+ achParamSpaceString
	...
	protected void writeMethodBody(MethodItem mi, int paramNum,
		...
		case MethodItem.METHODTYPE_PROPERTYPUTREF:
			...
			// ---- special processing should be done in the functions
			// write*MethodBody
			if (paramNum == 1) {
	 ...
	 *             the used Writer could throw an IOException
	 */
	protected void writeOutFunctionMethodBody(MethodItem mi, int paramNum)
			throws IOException {
		ParameterItem[] parameters = mi.getParameters();
		...
		for (int i = 0; i < paramNum; i++) {
			...
			// !p.getType().equals("SafeArray") ) {
			if (p.getDirection() == ParameterItem.DIRECTION_OUT) {
				w.write("\t\tVariant " + p.getVariantName()
						+ " = new Variant();\n");
				w.write("\t\tif( " + p.getJavaName() + " == null || "
						+ p.getJavaName() + ".length == 0 )\n");
				w.write("\t\t\t" + p.getVariantName() + ".putNoParam();\n");
				w.write("\t\telse\n");
				// 12/2005 hack to get get this to generate compileable code.
				// Hopefully someone will look at this later and make sure it is
				// correct
				if (p.getVariantPutMethod().equals("putVariantRef")) {
					w.write("\t\t\t" + p.getVariantName() + " = "
							+ p.getJavaName() + "[0];\n\n");
				} else if (p.getVariantPutMethod().equals("java.util.Date")) {
					w.write("\t\t\t" + p.getVariantName() + ".putDate("
							+ p.getJavaName() + "[0]);\n\n");
				} else {
					w.write("\t\t\t" + p.getVariantName() + "."
							+ p.getVariantPutMethod() + "(" + p.getJavaName()
							+ "[0]);\n\n");
				}
			}
		}
		...
		// values from the Varaints with to*(). This has to be done after the
		// call.
		...
		for (int i = 0; i < paramNum; i++) {
			...
			if (p.getDirection() == ParameterItem.DIRECTION_OUT) {
				...
				if (p.getVariantGetMethod().equals("toVariant")) {
					// 12/2005 hack to get get this to generate compileable
					// code.
					// Hopefully someone will look at this later and make sure
					// it is correct
					w.write("\t\t\t" + p.getJavaName() + "[0] = "
							+ p.getVariantName() + ";\n");
				} else {
					w.write("\t\t\t" + p.getJavaName() + "[0] = "
							+ p.getVariantName() + "."
							+ p.getVariantGetMethod() + "();\n");
				}
			}
		}
	...
	protected void writeFunctionMethodBody(MethodItem mi, int paramNum)
			throws IOException {
		w.write("\t\t");
	...
	protected void writeDateConversionCode() throws IOException {
		...
		w
				.write("\t= java.util.Calendar.getInstance().get(java.util.Calendar.ZONE_OFFSET);\n\n");
		/*
		 * w.write( "\tstatic java.util.Date comDateToJavaDate(double comDate)
		 * {\n"); w.write( "\t\tcomDate = comDate - 25569D;\n"); w.write(
		 * "\t\tlong millis = Math.round(86400000L * comDate) -
		 * zoneOffset;\n\n"); w.write( "\t\tjava.util.Calendar cal =
		 * java.util.Calendar.getInstance();\n"); w.write( "\t\tcal.setTime(new
		 * java.util.Date(millis));\n"); w.write( "\t\tmillis -=
		 * cal.get(java.util.Calendar.DST_OFFSET);\n\n"); w.write( "\t\treturn
		 * new java.util.Date(millis);\n"); w.write( "\t}\n\n");
		 */
		w
				.write("\tstatic double javaDateToComDate(java.util.Date javaDate) {\n\n");
		w
				.write("\t\tjava.util.Calendar cal = java.util.Calendar.getInstance();\n");
		w.write("\t\tcal.setTime(javaDate);\n");
		w
				.write("\t\tlong gmtOffset = (cal.get(java.util.Calendar.ZONE_OFFSET) + cal.get(java.util.Calendar.DST_OFFSET));\n\n");
	...
	/*
	 * protected String computeParamType( ParameterItem pi ) { String result =
	 * "new Variant(" + pi.getName() + ")"; if( pi.getType().equals("String" ) ||
	 * pi.isObject() || pi.isCustomType() ) result = pi.getName();
	 * 
	 * return result; }
	 */
