...
#define PROXY_FLD "m_pConnPtProxy"
   ...
   */
  JNIEXPORT void JNICALL Java_com_jacob_com_DispatchEvents_init3(JNIEnv *env,
   ...
   * Signature: ()V
   */
  JNIEXPORT void JNICALL Java_com_jacob_com_DispatchEvents_release(JNIEnv *env, jobject _this)
  {
...
#define IMPLTYPE_DEFAULTSOURCE \
  ...
  BOOL getClassInfoFromProgId(LPOLESTR bsProgId, LPTYPEINFO *pClassInfo)
  {
    ...
    CLSID clsid;
    GUID libid;
    if (FAILED(CLSIDFromProgID(bsProgId, &clsid)))
      return false;
    if (FAILED(StringFromCLSID(clsid, &bsProgId)))
      return false;
    ...
    if (FAILED(CLSIDFromString(bsLibId, &libid)))
      return false;
    // Try loading from registry information.
    ITypeLib *pITypeLib;
    if (FAILED(LoadRegTypeLib(libid, 1, 0, LANG_NEUTRAL, &pITypeLib)))
      return false;
  ...
  BOOL getClassInfoFromProgIdTypeLib(BSTR typeLib, LPOLESTR bsProgId, LPTYPEINFO *pClassInfo)
  {
    ...
    ITypeLib *pITypeLib;
    if (FAILED(LoadTypeLib(typeLib, &pITypeLib)))
      return false;
