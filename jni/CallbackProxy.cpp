...
CallbackProxy::CallbackProxy(JNIEnv *env,
		jobject aSinkObj) :
   // initialize some variables
	m_cRef(0), MethNum(0), MethName(NULL), MethID(NULL)
{
...
STDMETHODIMP CallbackProxy::QueryInterface(REFIID rid, void **ppv)
{
  if (rid == IID_IUnknown || rid == IID_IDispatch) 
  {
    *ppv = this;
    AddRef();
    return S_OK;
...
STDMETHODIMP CallbackProxy::GetIDsOfNames(REFIID riid,
...
STDMETHODIMP CallbackProxy::Invoke(DISPID dispID, REFIID riid,
...
void CallbackProxy::convertJavaVariant(VARIANT *java, VARIANT *com) {
...
STDMETHODIMP CallbackProxy::RegisterEvent(DISPID dispID, const char * methodName)
{
	int newMethNum = MethNum + 1;
	DISPID* newMethID = new DISPID[newMethNum];
	const char **newMethName = new const char*[newMethNum];
	
	if (0 != MethNum) {
		memcpy(newMethID, MethID, MethNum * sizeof(DISPID));
		memcpy(newMethName, MethName, MethNum * sizeof(char*));
		delete [] MethID;
		delete [] MethName;
		MethID = NULL;
		MethName = NULL;
	...
	newMethID[MethNum] = dispID;
	newMethName[MethNum] = methodName;
